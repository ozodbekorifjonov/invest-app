...
import {
  createMajorSectorAPI,
  deleteMajorSectorAPI,
  updateMajorSectorAPI,
} from '../../api/investApi';
import { toast } from 'react-toastify';

function MajorSector() {
  const [currentItem, setCurrentItem] = useState(null);
  const [itemName, setItemName] = useState('');
  const [show, setShow] = useState(false);

  const { isLoading, majorSectorList, getMajorSectorList } = useRecommends();
  ...
  const deleteItem = async (id) => {
    try {
      const res = await deleteMajorSectorAPI(id);
  ...
  const addItem = async () => {
    if (currentItem) {
      try {
        const res = await updateMajorSectorAPI(currentItem.id, itemName);
        toast.success(res.data.message);
      } catch (error) {
        toast.error(error.message);
      }
      ...
      try {
        await createMajorSectorAPI(itemName);
      } catch (error) {
        toast.error(error.message);
      }
    }
    ...
    <div className="row">
      ...
      <Modal show={show} onHide={handleCloseModal}>
        <Modal.Header closeButton>
          <Modal.Title>{currentItem ? 'Edit major sector' : 'Add new major sector'}</Modal.Title>
        </Modal.Header>
        <Modal.Body>
