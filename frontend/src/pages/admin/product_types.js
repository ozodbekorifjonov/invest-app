...
function ProductTypes() {
  ...
  const deleteItem = async (id) => {
    try {
      const res = await deleteProductTypeAPI(id);
  ...
  const addItem = async () => {
    if (currentItem) {
      try {
        const res = await updateProductTypeAPI(currentItem.id, itemName);
        toast.success(res.data.message);
      } catch (error) {
        toast.error(error.message);
      }
      ...
      try {
        await createProductTypeAPI(itemName);
      } catch (error) {
        toast.error(error.message);
      }
    }
