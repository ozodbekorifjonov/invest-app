...
import { createInstrumentAPI, deleteInstrumentAPI, updateInstrumentAPI } from '../../api/investApi';
...
function Instruments() {
  ...
  const { isLoading, instrumentList, getInstrumentsList } = useRecommends();
  ...
  const deleteItem = async (id) => {
    try {
      const res = await deleteInstrumentAPI(id);
  ...
  const addItem = async () => {
    if (currentItem) {
      try {
        const res = await updateInstrumentAPI(currentItem.id, itemName);
        toast.success(res.data.message);
      } catch (error) {
        toast.error(error.message);
      }
      ...
      try {
        await createInstrumentAPI(itemName);
      } catch (error) {
        toast.error(error.message);
      }
    }
    ...
    <div className="row">
      ...
      <Modal show={show} onHide={handleCloseModal}>
        <Modal.Header closeButton>
          <Modal.Title>{currentItem ? 'Edit instrument' : 'Add new instrument'}</Modal.Title>
        </Modal.Header>
        <Modal.Body>
