...
import { createRiskRatingAPI, deleteRiskRatingAPI, updateRiskRatingAPI } from '../../api/investApi';
...
function RiskRating() {
  ...
  const { isLoading, riskRatingList, getRiskRatingList } = useRecommends();
  ...
  const deleteItem = async (id) => {
    try {
      const res = await deleteRiskRatingAPI(id);
  ...
  const addItem = async () => {
    if (currentItem) {
      try {
        const res = await updateRiskRatingAPI(currentItem.id, itemName);
        toast.success(res.data.message);
      } catch (error) {
        toast.error(error.message);
      }
      ...
      try {
        await createRiskRatingAPI(itemName);
      } catch (error) {
        toast.error(error.message);
      }
    }
    ...
    <div className="row">
      ...
      <Modal show={show} onHide={handleCloseModal}>
        <Modal.Header closeButton>
          <Modal.Title>{currentItem ? 'Edit risk rating' : 'Add new risk rating'}</Modal.Title>
        </Modal.Header>
        <Modal.Body>
